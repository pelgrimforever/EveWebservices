...
public class RSLoadrouteTest {
    ...
    @InjectMocks
    private RSLoadroute rsloadroute = new RSLoadroute();    
    
    public RSLoadrouteTest() {
    }
    ...
    public void setUp() {
        ...
        systems.add(system);
        try {
            doReturn(systems).when(loadroute_usecases_mock).Loadroute_withswagger_usecase(any(Loadroute_parameters.class));
        }
        catch(DBException e) {
        }
    }
    ...
    public void findroute_test() {
        try {
            String result = rsloadroute.post(jsonpayload);
